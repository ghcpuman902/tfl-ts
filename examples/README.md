...
| **Bus** | Route number chip | **Do not** use tube colour helpers | Discover stops → expand `490G…` hubs if search returns them → poll boardable `/^490\d/` IDs → arrivals row: `[chip] [destination] [countdown]` |
...
4. **Bus ≠ tube.** Route chips are generic; stop letter is a small badge; countdown is `Due` / `N min` from `timeToStation`. Use `stop.towards` / `stop.compassPoint` from `get` / `getByGeoPoint` — not Prediction `bearing`.
5. **Prefer boardable bus stops for arrivals.** Boarding IDs match `/^490\d/` (not `/^490/` — `490G…` are area hubs with no live arrivals). Name search: call `client.stopPoint.searchBusStops(query)` so hubs are expanded. `search({ modes: ['bus'] })` is the raw TfL mix.
