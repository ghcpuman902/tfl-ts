...
| vgpu.sh publishes `agents.md` for cross-tool discovery | tfl-ts only ships `CLAUDE.md`; tools that look specifically for `AGENTS.md` find nothing | **Adopt, minimally.** Add root `AGENTS.md` as a short pointer to `CLAUDE.md` rather than a duplicate. tfl-ts already has a strong "single source of truth, no generated drift" convention (see `generated.meta.json` in the maintainer skill); duplicating a 90-line doc into two files that must be kept in sync by hand would violate that convention for no real benefit, since `tfl docs cat` makes either file trivially reachable anyway. |
...
| `vgpu doctor`, `vgpu examples pull`, MCP `docs`/`examples` tools | Environment diagnostics (`TFL_APP_KEY` set? reachable?) and copy-paste example discovery from the CLI, and exposing the same doc manifest over the existing MCP server | **Future work**, listed but not implemented here to keep this change to a single, reviewable, low-risk addition. `tfl doctor` and an MCP `docs` tool are natural follow-ups once `tfl docs` proves useful; `examples pull` is lower priority because tfl-ts already ships `examples/` directly in the npm package (no hosted gallery to fetch from). |

## What shipped in this change

1. `src/bin/docs.ts` — a manifest of the Markdown files tfl-ts already ships (`CLAUDE.md`, `AGENTS.md`, `docs/agent.md`, `docs/mcp.md`, `docs/REALTIME.md`, `docs/MIGRATION-v2.md`, `examples/README.md`, `.claude/skills/tfl-ts/SKILL.md`, `ERROR.md`, `CHANGELOG.md`) plus `listDocs` / `catDoc` / `findDocs` / `grepDocs` functions, wired into `tfl docs` in `src/bin/tfl.ts`.
2. `AGENTS.md` — a one-line pointer to `CLAUDE.md`.

Net effect: an agent working purely from a shell — no repo checkout, context already compacted, only `tfl` on the `PATH` — can run `npx tfl-ts docs cat CLAUDE.md` (or `find`/`grep` if it doesn't remember the exact filename) and get back the same conventions a fresh session would have had, in one offline call. That is the same trade tfl-ts already made for the *API*: `tfl raw`/`tfl list` mean an agent never has to guess raw endpoint names either.
