...
| `vgpu check <file.wgsl>` — instant, no-GPU shader validation | tfl-ts already has an instant, no-browser check for the *live* API surface (`tfl raw`, `tfl smoke`). The closest un-covered case is validating line/stop/mode IDs offline against bundled metadata before making a live call | **Adopted (later).** `tfl check --line` / `--mode` and `checkLineIds` / `checkModeNames` on `tfl-ts/meta` validate bundled `Lines` / `Modes` with no client and no key. Display-case `'Central'` fails and suggests `central`. |
